...
_base_service_client: Optional[AsyncClient] = None
...
async def init_service_client() -> None:
    ...
    if _supabase_url and _supabase_service_key and _base_service_client is None:
        ...
        try:
            _base_service_client = await create_async_client(_supabase_url, _supabase_service_key)
            logger.info("Supabase async base service client initialized successfully.")
        except Exception as e:
            logger.error(f"Failed to initialize Supabase service client: {e}", exc_info=True)
            _base_service_client = None
    elif not _supabase_service_key:
        logger.warning("SUPABASE_SERVICE_ROLE_KEY not provided. Base service client not initialized.")
...
async def get_base_supabase_client() -> AsyncClient:
    ...
    try:
        ...
        logger.error(f"Failed to create base Supabase client instance: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Failed to initialize database client.",
        )

async def get_supabase_service_client() -> Optional[AsyncClient]:
    """
