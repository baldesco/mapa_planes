...
async def _delete_storage_object(path: str, db_service: AsyncClient) -> bool:
    ...
    try:
        ...
        await db_service.storage.from_(settings.SUPABASE_BUCKET_NAME).remove([path_to_delete])
        return True
    except Exception as e:
        logger.error(f"CRUD: Exception during storage deletion of '{path}': {e}", exc_info=True)
        return False
...
async def _get_visits_for_place_ids(
    ...
    try:
        response = await db.table(VISITS_TABLE).select("*").in_("place_id", place_ids).execute()
        ...
        logger.error(f"CRUD Helper: Unexpected error fetching visits: {e}", exc_info=True)
        return visits_by_place_id
...
async def _get_tags_for_place_ids(
    ...
    try:
        response = await db.table(PLACE_TAGS_TABLE).select("place_id, tags(*)").in_("place_id", place_ids).execute()

        if response.data:
            for item in response.data:
                place_id_from_tag = item.get("place_id")
                tag_data = item.get("tags")
                if place_id_from_tag and tag_data and place_id_from_tag in tags_by_place_id:
                    try:
...
async def create_place(
    ...
    try:
        ...
        if place.latitude is not None and place.longitude is not None and place.timezone_iana is None:
...
async def get_places(
    ...
    try:
        query = db.table(TABLE_NAME).select("*").eq("user_id", str(user_id)).order("created_at", desc=True)
        ...
        for p_data in place_data_list:
            try:
                ...
                logger.error(f"CRUD: Pydantic validation error for place ID {p_data.get('id')}: {validation_error}")
...
async def get_place_by_id(
    ...
    try:
        ...
        if response.data:
            ...
            place_data["tags"] = await crud_tags.get_tags_for_place(db=db, place_id=place_id)
        ...
        logger.error(f"CRUD: Exception in get_place_by_id for ID {place_id}: {e}", exc_info=True)
        return None
...
async def update_place(
    ...
    update_data = place_update.model_dump(exclude_unset=True, exclude_none=False, exclude={"tags"})

    if "latitude" in update_data or "longitude" in update_data:
        new_lat = update_data.get("latitude", current.latitude)
        new_lon = update_data.get("longitude", current.longitude)
        if new_lat is not None and new_lon is not None:
            update_data["timezone_iana"] = await get_timezone_from_coordinates(new_lat, new_lon)

    if update_data:
        update_data["updated_at"] = datetime.now(timezone.utc).isoformat()
        update_data.pop("deleted_at", None)
        await db.table(TABLE_NAME).update(update_data).eq("id", place_id).eq("user_id", str(user_id)).execute()
...
async def delete_place(
    ...
    try:
        ...
        response = await db.table(TABLE_NAME).update({
            "deleted_at": delete_time, 
            "updated_at": delete_time
        }).eq("id", place_id).eq("user_id", str(user_id)).execute()
        return bool(response.data)
    except Exception as e:
        logger.error(f"CRUD: General Exception in soft delete for ID {place_id}: {e}", exc_info=True)
        return False
...
async def _update_place_status_after_visit_change(
    ...
    try:
        visits_response = await db.table(VISITS_TABLE).select(
            "visit_datetime, rating, review_text, review_title"
        ).eq("place_id", place_id).eq("user_id", str(user_id)).execute()
        ...
        if has_future_visits:
            ...
            if has_reviewed_past_visit:
                ...
                current_place_response = await db.table(TABLE_NAME).select("status").eq("id", place_id).eq("user_id", str(user_id)).maybe_single().execute()
...
async def upload_place_image(
    ...
    """
    logger.info(f"CRUD: Uploading image '{file.filename}' for place {place_id}, user {user_id}")
    try:
        ...
        logger.error(f"CRUD: Exception uploading image for place {place_id}: {e}", exc_info=True)
        return None